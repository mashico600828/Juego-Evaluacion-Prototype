...
var initialPos;
var espera=0;
var score=0;    // acumulador de puntaje
var mov=0;     // contador de movimientos del juegador

var min=2;     // limite de tiempo en minutos
var seg=0;     // contador de segundos
...
$(".btn-reinicio").click(function(){
  ...
  $(".panel-tablero").show();    // muestro el tablero
  $(".time").show();             // muestro el cronometro

  $("#score-text").html("0")     // inicializo el marcador 
  $("#movimientos-text").html("0") // inicializo el contador de movimientos
  $(this).html("REINICIAR")        // cambio el titulo del boton
  clearInterval(intervalo);
  clearInterval(eliminar);
  clearInterval(newdulces);
  clearInterval(tiempo);
  min=2;  // limite de tiempo de juego 2 minutos
  seg=0;  // contador de segundos a 0
  borrartotal()
  intervalo=setInterval(function(){tablero()},600)
...
function borrartotal()
{
  for(var j=1;j<8;j++)
  {
    $(".col-"+j).children("img").detach();    // quito todos los elementos de imagenes por columnas 
  }
}
...
function tablero()
{
  ...
  if(i<8)
  {
    for(var j=1;j<8;j++)
    {
      ...
      {
        numero=Math.floor(Math.random() * 4) + 1 ;    // genero el numero randomico entre 1 y 4
  ...
  if(i==8)
  {
    clearInterval(intervalo);   //desactivar funcion tablero()
    eliminar=setInterval(function(){eliminardulces()},150)  //activar funcion eliminarhorver
  }
}
//------------------------------------------------------------------------------
...
{
  matriz=0;
  rbh=horizontal()  //funcion para busqueda dulces en forma horizontal o filas
  rbv=vertical()    //funcion para buscar dulces en forma vertical o columnas
  ...
  if(rbh==1 || rbv==1)
  {
    ...
    $(".activo").hide("pulsate",1000,function(){
      ...
      $(".activo").remove("img")
      score=score+scoretmp;          // actualizo el puntaje
      $("#score-text").html(score)  // mostrar puntuacion
    })
  }

  if(rbh==0 && rbv==0 && matriz==49)
  {
    $(".elemento").draggable({  // si el elemnto esta listo a ser movido y asigando draggable en on
      disabled: false,
      ...
      start: function(event, ui){
        mov=mov+1;                 // actualizo el numero de movimientos
        $("#movimientos-text").html(mov)    // muestro el numero de movimientos
      }
    });
  }

  $(".elemento").droppable({     // termino el intercmbio de los elementos 
    drop: function (event, ui) {
      ...
      rbh=horizontal()  //ejecuto la funcion busqueda dulces horizontal
      rbv=vertical()    //ejecuto la funcion buscar dulces vertical
      if(rbh==0 && rbv==0)
      {
...
function nuevosdulces()
{
  ...
  if(maximo!=0)
  {
    ...
    for(var j=1;j<8;j++)
    {
      if(contador>(maximo-lenres[j-1]))
      {
        numero=Math.floor(Math.random() * 4) + 1 ;  // genero nuevo numero aleatorio entre 1 y 4 para poner la imagen
        imagen="image/"+numero+".png";
...
function horizontal()
{
  ...
  for(var j=1;j<8;j++)
  {
    for(var k=1;k<6;k++)
    {
      ...
      if((res1==res2) && (res2==res3) && (res1!=null) && (res2!=null) && (res3!=null))  // verifico la condicion
      {
          ...
          bh=1;   // si cumple la condicion enciendo el flag o indicador
      }
    }
  }
  return bh;
  ...
  for(var l=1;l<6;l++)
  {
    for(var k=1;k<8;k++)
    {
      ...
      if((res1==res2) && (res2==res3) && (res1!=null) && (res2!=null) && (res3!=null))  // verifico la condicion 
      {
          ...
          bv=1;   // si cumple la condicion enciendo el flag o indicador
      }
    }
  }
  return bv;
